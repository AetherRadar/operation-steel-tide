...
| `OpticMount` | Quaternius `axmc.glb` (`Sniper Rifle-TKaBjAEofL.glb`), object `SniperRifle_3` | Complete glass-bearing scope component fitted to the top rail |
...
build asserts pre-export parentage, transforms, and non-empty triangle geometry
under all four attachment nodes. Godot's `--validate-combat-models` diagnostic
then validates the imported GLB hierarchy, attachment geometry, configuration
truth table, and active muzzle/reticle alignment. The final GLB contains 18 mesh
instances, approximately 12,430 imported vertices, and 10,629 triangles.
