...
acquired on 2026-08-28. Official source pages and license metadata were
rechecked on 2026-08-28. Cache paths below are relative to
`JIANGHAI_ACQUISITION_ROOT`; paths beginning with `assets/` are
repository-local sources that predate this map composition.
...
## Final source and runtime hashes
...
procedurally generate the city layout. It deterministically reapplies the
documented factory-frontage substitution, sign cleanup, material tuning, and
export policy. The packed `.blend` remains the authoritative source for the
final hand-edited composition.

The delivered Poly Haven bundles remain in the external cache as 1K glTF,
...
that delivered set as 36 adapted facade objects. All listed inputs are CC0, so
no non-redistributable marketplace payload is present in the delivered files.

The repository-local Kloppenheim 06 (Pure Sky) HDRI is a separate runtime sky
input loaded by `JianghaiOldCityAtmosphere`. It is not an acquisition-cache
dependency, is not packed into the authoritative `.blend`, and is not embedded
in the runtime map GLB.
...
five market shops, two rear houses, five Chinese red lamps, and the market,
pawnshop, and factory authored-prop clusters described above. The
...
The same audit recorded seven required runtime anchors:
...
`MunicipalTreasuryManifestTerminalVisual`. In the final saved DCC scene, the
Municipal terminal root's duplicate 180-degree rotation is removed and its
...
Kloppenheim 06 (Pure Sky) is a separate CC0 runtime HDRI acquired on
2026-08-28; `JianghaiOldCityAtmosphere` loads it from `assets/textures/`, and it
is not embedded in either final DCC/runtime map binary.
