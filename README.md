...
- **另一套完整对局：** 在十二张地图中选择目前可玩的 Tideforge Arena、Harbor Locks 或 Tideglass Reactor，再进入包含手动购买、半场换边、加时、安放、拆除和职责型战术 AI 的 MR12 5v5 对局。
...
- 鼠标左键射击，或在近战状态下连续输入三段斩击；鼠标右键通过已安装的瞄具瞄准，`R` 执行完整弹匣换弹
...
- `csharp/DemolitionArenaLayout*.cs`、`DemolitionArenaBuilder*.cs` 和 `DemolitionArenaRuntime.cs`：Tideforge、Harbor Locks 与 Tideglass Reactor 的几何数据、多资产包正式场景组合、不可见玩法碰撞、激活隔离、路线平衡、长距离转点和小地图标记。
