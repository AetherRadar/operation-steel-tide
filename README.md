...
This is still an art-production work in progress, not a finished commercial game. The deployment preview now uses a redistributable authored tactical character, while several field characters, weapons, vehicles, and world modules still need the same Blender-led replacement pass. The useful question is whether the systems hold together: try a run, inspect the source, and [report the first thing that breaks](https://github.com/AetherRadar/operation-steel-tide/issues). If you want to follow the refactor and future operations, star the repository.
