...
The same lobby now includes a deployment-map selector. `MAP 01 // FREIGHT TERMINAL` and `MAP 02 // BLACKWATER REFINERY` are playable extraction operations, while `MAP 03 // ORBITAL COMPLEX` remains visibly locked. Selecting a different playable map stages the squad/loadout and reloads the world, so only one 340 m x 320 m extraction map is resident at a time.

Blackwater Refinery recomposes 26 CC0 source scenes from Kenney and Poly Haven into 90 placed industrial buildings, tanks, chimneys, road barriers, and military crates. Its intake yard, twin pump districts, cracking yard, and turbine compound have independent loot and garrison routes around a clear central vehicle corridor. Imported visuals use cached scenes, distance culling, reduced prop shadows, and one scaled box collision proxy per placement; the map-specific runtime check currently stays below 900 nodes, 125 static bodies, 500 mesh instances, and 20 lights.

The Operations Office also launches the separate demolition match from a twelve-map pool. Its briefing shows one map at a time with previous/next controls: `TIDEFORGE ARENA` and `HARBOR LOCKS` are playable, while the other ten slots remain visibly locked until their geometry ships. Tideforge splits its sites across an open foundry and enclosed assembly hall. Harbor Locks recomposes Kenney's CC0 City Kit (Industrial) models into a lock-gate district with pump stations, control buildings, two long quayside channels, three attack routes, and hard-cover rotations. Both maps keep the sites 77 m apart and attack travel times within twelve percent. Each match is MR12 5v5: the player plus four AI teammates against five opponents, first to 13 rounds, halftime side swap with wallets reset to $800, and a win-by-two overtime that swaps sides every four rounds. A 15-second buy phase freezes combat before every round and presents sidearms, primary weapons, armor, and grenades with exact prices. The opening $800 can buy a P226 or M1911 but cannot buy a primary; confirming deducts the validated total once, while timeout accepts the current affordable selection or starts knife-only. Round wins pay $3,000, losses start at $1,900 plus a $500 loss-streak escalation, planting or defusing pays $300, and wallets cap at $9,000. Opponents purchase from the same price ladder, while extraction progression and its wallet remain isolated. When the player squad defends, enemy AI picks a carrier who walks a route, plants, and the remaining attackers hold angles; attackers defuse through cover while defenders rotate.
...
`--validate-refinery-map` boots Blackwater Refinery directly and verifies all 90 authored model placements, CC0 source coverage, scene caching, distance culling, box-only collision proxies, loot/garrison/minimap integration, central vehicle lanes, and strict node/body/mesh/light budgets. `--capture-refinery-map` saves both an overhead composition frame and a player-height road view.
