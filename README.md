...
1. 启动游戏，点击 **QUICK EXTRACTION**、**DEMOLITION** 或 **TRAINING RANGE**。
...
入口：[当前中文实机画面](#当前中文实机画面) · [下载最新发行版](https://github.com/AetherRadar/operation-steel-tide/releases/latest) · [完整操作](#操作方式)

## 玩法亮点

- **真正承担职责的小队：** 五种干员职业、跟随/驻守/移动命令、职业技能、战斗、救援，以及联机玩家掉线后的 AI 自动接管。
- **有代价的撤离循环：** 操作实体任务目标，搜索建筑和阵亡干员，抵御增援，并且只存入高于部署基准的撤离收益。
- **另一套完整对局：** 在十二张地图中依次选择目前可玩的 Bazaar Crossing、Tideglass Reactor、Tideforge Arena 或 Harbor Locks，再进入包含手动购买、半场换边、加时、安放、拆除和职责型战术 AI 的 MR12 5v5 对局。
...
脚本会在本机 Godot keystore 目录生成 debug 密钥，输出 `dist/OperationSteelTide-1.4.1-android-arm64-debug.apk` 及其 SHA-256 校验文件。release 构建需要先设置 `GODOT_ANDROID_KEYSTORE_RELEASE_PATH`、`GODOT_ANDROID_KEYSTORE_RELEASE_USER` 和 `GODOT_ANDROID_KEYSTORE_RELEASE_PASSWORD`，再将 `-Mode` 改为 `Release`；正式发布到 Google Play 仍需单独配置 release keystore 与 AAB 流程。
...
双击 `START_GAME.bat`。启动器会通过 `GODOT_MONO`、PATH 或默认下载目录查找兼容的 Godot 4.6 Mono。它会检查 C# 源文件、Godot 资源和本地任务服务是否真的有变化：没有变化时直接复用 `.godot` 导入缓存、C# DLL 和 Go 服务，因此后续启动接近直接打开游戏的速度；有变化时仍会自动增量构建并导入，最新改动不会丢失。全新检出第一次运行时需要导入正式模型和纹理，因此可能稍慢。同一检出目录现在可以多开：共享的构建和导入准备阶段会依次排队，准备完成后各个 Godot 游戏进程可并行运行，并分别记录日志。每次实际启动都会把独立的导入和运行日志，以及本次启动器自建后端的日志写入 `logs/startup/<run-id>`，并在启动前显示该目录。日志清理不会删除启动器仍在运行的目录，并保留最近 20 个非活动运行目录；即使 Godot 自身返回 0，只要日志出现顶层错误，本次启动仍会明确失败。

如果刚替换了资源但文件时间没有更新，或想主动重建全部准备步骤，可运行 `START_GAME.bat --force-prepare`；这个参数只给启动器，不会传给游戏。
...
- `1`、`2`、`3` 分别切换主武器、副武器和手枪，`4` 拿出当前装备的近战武器，`5` 选择破片手雷，`6` 选择当前战术道具；同时持有烟雾弹和燃烧弹时，再按一次 `6` 可在两者之间切换。手雷或战术道具库存为零时，相应槽位会隐藏；鼠标指针可用时也可以点击右下角槽位
...
行动办公室还可以从十二张地图组成的池中启动独立爆破比赛。任务简报通过前后按钮一次展示一张地图：`BAZAAR CROSSING`、`TIDEGLASS REACTOR`、`TIDEFORGE ARENA` 和 `HARBOR LOCKS` 当前依次位于前四项，其余八个位置在对应几何资源完成前保持可见但锁定。Tideglass Reactor 现作为 `MAP 02`：136 米 x 112 米战区由紧凑中路和两条较长侧翼组成，施工院 A 与旧反应堆装卸场 B 是仅有的两个爆能器点，相距 95.52 米。41 栋主要建筑覆盖五个区域，其中 30 栋属于本次扩建；施工塔楼与吊机、完整砖砌反应堆大楼、市政十字路口、两座不同的边界闸门、住宅办公楼、橙灰色模块化厂房和街道设施共同形成连续街区，不再使用脱离建筑的孤立隔板。26 条装饰模型引用与 52 条带碰撞正式道具引用合计 78 条，去重后为来自八套 CC0 来源的 70 条正式场景路径；重复引用仅用于复用完整建筑模型。三条进攻主线和六条地面路线均通过角色胶囊体与运行时物理验证，三路最小间距为 35.22 / 9.90 / 30.77 米且重叠率均为 0。严格密度采样将最长水平视线限制在 58.00 米，最大开放空间直径为 21.78 米；地图构建低于 6000 毫秒，全部掩体点、策略点、爆点覆盖和双方出生/爆点实体视线遮挡均通过诊断。Tideforge 的两个目标点分别位于开放式铸造厂和封闭装配车间。Harbor Locks 将 Kenney 的 CC0 City Kit (Industrial) 模型重新组合成一片船闸区，包含泵站、控制建筑、两条狭长岸边水道、三条进攻路线和硬掩体转点路线。Bazaar Crossing V2 则推翻旧有的空旷镜像平面：136 米 x 112 米战区内，A 是可进入的双层商院，B 是完整覆顶的仓库市集，四段室内房间组成 S 形 Mid，北侧回防在屋顶市场中连续折叠；A Gallery、B Balcony 与 Mid Mezzanine 各自只影响一个局部区域，并分别通过两座楼梯接入三维 AI 路线。

每场比赛采用 MR12 5v5：玩家和四名 AI 队友对阵五名敌人，先赢 13 回合者获胜；中场交换阵营并将资金重置为 $800；加时要求净胜两回合，且每四回合交换阵营。每回合开始前有 15 秒购买阶段，期间战斗冻结，界面按准确价格提供副武器、主武器、护甲、破片手雷、烟雾弹和燃烧弹；M24 也进入爆破购买阶梯，并限制每支 AI 队伍在同一回合最多一名狙击手。初始 $800 可购买 P226 或 M1911，但买不起主武器；确认购买只扣除一次校验后的总价，超时则接受当前可负担的选择，或仅持刀开始。回合胜利奖励 $3,000；失败基础奖励为 $1,900，并按连败增加 $500；安放或拆除奖励 $300；资金上限为 $9,000。敌人使用相同价格阶梯购买装备，爆破模式与撤离模式的进度和资金彼此隔离。AI 的基础准度不因爆破强化而提高；提升来自掩体选择、看得见敌人才中断目标、最后安放/拆除窗口的硬性提交，以及按进攻、回防、守包和确认掩体分别使用烟雾弹、燃烧弹与破片手雷。带枪干员被击败后，枪械会从尸体战利品中分离并以正式模型直接落地，玩家在近距离且视线畅通时可按 `F` 拾取或交换。队友和敌人还会使用地图中正式标记的梯子上下楼层，普通移动不会增加高频路径扫描。
