...
- **A separate demolition match:** pick from a twelve-map pool (TIDEFORGE ARENA playable today), deploy into an MR12 5v5 — first to 13 rounds, halftime side swap, win-by-two overtime — on an $800 opening wallet with round rewards, loss-streak bonuses, and automatic eco/full buys, and fight AI that assigns entry, support, recon, anchor, rotation, retake, cover, flank, plant, and defuse duties from the live team state.
...
The Operations Office also launches the separate demolition match from a twelve-map pool: `TIDEFORGE ARENA` is playable, and the other eleven slots stay visibly locked until their geometry ships. The industrial layout splits the sites across the arena diagonal — an open cast-house A site in the southwest and an enclosed assembly-hall B site in the northeast, 77 m apart — with three attack routes, a central rotation lane, and attack travel times kept within twelve percent. Each match is MR12 5v5: the player plus four AI teammates against five opponents, first to 13 rounds, halftime side swap with wallets reset to $800, and a win-by-two overtime that swaps sides every four rounds. Round wins pay $3,000, losses start at $1,900 plus a $500 loss-streak escalation, planting or defusing pays $300, wallets cap at $9,000, and squads above $4,400 auto-buy their chosen loadout while lighter wallets buy the $2,400 eco kit. When the player squad defends, enemy AI picks a carrier who walks a route, plants, and the remaining attackers hold angles; attackers defuse through cover while defenders rotate. Extraction progression stays isolated.
...
- `csharp/DemolitionMatchState.cs`, `DemolitionEconomy.cs`, and `DemolitionStrategyPlanner.cs`: pure MR12 scoring with halftime swaps and win-by-two overtime, the $800 round economy with rewards/streaks/caps, and role-, health-, range-, survival-, and position-aware team assignments.
- `csharp/DemolitionMapCatalog.cs`: the twelve-map demolition pool with per-map availability for open map selection.
...
`--validate-demolition` verifies the Operations Office entry, role and custom loadout selection, primary/sidearm magazine persistence, isolated economy and extraction systems, the 5v5 squad fill, opening and post-plant AI duties, planting, physical AI defusing, the tactical AI layer (combat-first arbitration with hysteresis, mid-channel guard, clock-pressure site switching, detour routing, squad post anchoring), the $800 eco opening buy, round rewards, round scoring/reset, the round-13 halftime side swap with enemy-carrier planting and player defusing, MR12/13-win/overtime win-by-two rules, and the economy reward table. `--validate-demolition-arena` checks activation, 108 m spawn separation, roughly 80-90 m A/B approaches, the roughly 113 m rotation, balanced travel, capsule-clear routes, blocked spawn sightlines, 77 m site separation, site placement, localized minimap markers, and spatial isolation. `--validate-demolition-briefing` verifies scene loading, required bindings, English and Chinese synchronization, the twelve-map pool with locked-map rejection, role/loadout/map selection without deployment, and back/deploy intent signals.
