...
- `1` draws the primary weapon, `2` the secondary weapon, `3` the sidearm, `4` the equipped melee weapon, `5` a frag grenade, and `6` the current utility item (a smoke grenade); grenade and utility slots stay hidden at zero inventory, and the lower-right slots can also be clicked while the pointer is available. 中文：按 `1`/`2`/`3` 切换主武器、副武器和手枪，按 `4` 切换至当前装备的近战武器，按 `5`/`6` 选择破片手雷和当前战术道具。
- Left mouse fires or chains up to three melee cuts, right mouse aims through the installed optic, and `R` performs a full magazine reload. 中文：鼠标左键射击，或在近战状态下连续输入三段斩击；鼠标右键瞄准，`R` 换弹。
...
部署和背包人物预览使用 BAMEN 的 CC BY 4.0 绑定军人模型，经仓库内 Blender 流程标准化并以克制的战术材质呈现。第一人称系统现支持三种项目原创近战风格：战术短刀、更重且攻击距离更长的斩马刀，以及出刀更快的天玄刀。按 `4` 拔出当前装备的近战武器，连续点击鼠标左键可衔接三段斩击。每种武器都有独立的拔刀花式和攻击节奏，刀光与多点扫掠判定会跟随实际刀刃轨迹。
