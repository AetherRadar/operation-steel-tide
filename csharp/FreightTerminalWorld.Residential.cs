...
public partial class FreightTerminalWorld
{
        ...
        Godot.Material trim)
    {
        ...
        _lowPolyBuildingArtBuilder.BuildResidentialDetails(
            tower,
            diversityProfile,
            spec.Footprint,
    ...
    private async void ValidateResidentialCommunity()
    {
        ...
        var hedgeBlockers = new List<string>();
        foreach (var node in hedgeColliders)
        {
            ...
            {
                hedgeBlockers.Add(
                    $"{hedge.GetParent()?.Name}/{hedge.Name}:{(hedgeHit.Collider as Node)?.Name ?? "none"}");
            }
        }
