...
public partial class FreightTerminalWorld
{
    ...
    private bool IsLocalAiSquadCommandSession
        => SquadCommandRules.CanExposeAiCommands(
            _demolitionMode,
            IsExtractionNetworkMatch,
            _squadDeployed,
            _squadMates.Count(IsLocalAiMate));

    private bool IsAllAiSquadSession
    {
        get
        {
            if (!_squadDeployed)
            ...
            return _squadMates
                .Where(mate => GodotObject.IsInstanceValid(mate))
                .All(mate => !mate.IsHumanProxy);
        }
    }

    private bool ShouldFailLocalPlayerOnSecondDown()
        => IsInstanceValid(_player)
            && _player.IsDead
            && SquadReviveRules.ShouldFailExtractionOnSecondDown(
                _demolitionMode,
                IsExtractionNetworkClient,
                _player.ReviveUsed,
                IsAllAiSquadSession);

    private bool CanEliminatedPlayerCommandLocalAi
        => IsLocalAiSquadCommandSession
            && _localPlayerEliminated
            && IsInstanceValid(_player)
            && _player.IsDead
            && _squadMates.Any(IsLivingLocalAiMate);

    private bool CanLocalPlayerIssueAiSquadCommands
        => !_missionEnded
            && IsLocalAiSquadCommandSession
            && !_localPlayerDowned
            && (!_player.IsDead && !_player.UiLocked
                || CanEliminatedPlayerCommandLocalAi);

    private static bool IsLocalAiMate(SquadMate mate)
        => GodotObject.IsInstanceValid(mate)
            && !mate.IsHumanProxy
            && !mate.IsNetworkProxy;

    private static bool IsLivingLocalAiMate(SquadMate mate)
        => IsLocalAiMate(mate)
            && mate.IsInsideTree()
            && !mate.IsDowned
            && !mate.IsBodyBag
    ...
    internal Node3D ResolveSquadFollowAnchor()
    {
        if (!CanEliminatedPlayerCommandLocalAi)
        {
            return _player;
        }
        var anchor = _squadMates
            .Where(IsLivingLocalAiMate)
            .OrderBy(mate => mate.SquadSlot)
            .FirstOrDefault();
        return anchor is null ? _player : anchor;
    }
    ...
    private void BuildSquadSystem()
    {
        ...
        _hud.SquadOrderRequested += TryIssuePlayerSquadOrder;
        _hud.SquadFireStanceRequested += TryTogglePlayerSquadFireStance;
    ...
    private ulong _abandonedAiReviveTargetId;
    private float _abandonedAiReviveTargetExpiry;
    private float _aiReviveRetryClock;
    private ulong _aiRevivePreparationTargetId;
    private float _aiRevivePreparationExpiry;
    private bool _aiRevivePreparationCompletionGraceUsed;
    private readonly Dictionary<ulong, float> _aiReviveTargetRetryCooldowns = new();
    ...
    private const float AiRevivePreparationTimeout = 10.0f;
    private const float AiRevivePreparationCompletionGrace = 3.0f;
    // A route that made no progress for 15 seconds should not repeatedly pull
    // eliminated-leader survivors away from evacuation. The target remains
    // retryable later in a long raid, but one full extraction window gets priority.
    private const float AiReviveTargetRetryCooldown = 60.0f;
    private const float AiReviveNoRescuerRetryCooldown = 2.5f;
    ...
    private void UpdateSquadReviveAi(float delta)
    {
        ...
        AdvanceAiReviveRetryClock(delta);
        if (_aiReviveTarget is { } pendingTarget
            && IsInstanceValid(pendingTarget.CombatNode)
            && pendingTarget.CanBeRevived
            && !TryMeasureNearestAiReviver(pendingTarget, honorFailedRevivers: true, out _))
        {
            // Do not retain a casualty forever while every survivor is too hurt,
            // disabled, or has already failed this route. A short
            // per-casualty cooldown lets sustainment run and allows other casualties
            // or extraction to make progress before this target is reconsidered.
            PutAiReviveTargetOnCooldown(pendingTarget, AiReviveNoRescuerRetryCooldown);
            ClearLeaderReviveAi();
        }
            ...
            || !CanAiRevive(_leaderReviver, preferredTarget))
        {
            ...
            if (nearest is null)
            {
                PutAiReviveTargetOnCooldown(
                    preferredTarget,
                    AiReviveNoRescuerRetryCooldown);
                ClearLeaderReviveAi();
                return;
            }
        ...
        if (!hasReviveAccess)
        {
            ...
            if (_reviverNoProgressTime >= AiReviveNoProgressTimeout)
            {
                ...
                PutAiReviveTargetOnCooldown(target, AiReviveTargetRetryCooldown);
                ClearLeaderReviveAi();
                return;
            }
        ...
        if (revived)
        {
            ResetAiReviveAbandonment(target);
        }
    ...
    private ISquadCombatant? FindAiReviveTarget()
    {
        if (_localPlayerDowned && IsInstanceValid(_player) && _player.CanBeRevived
            && !IsAiReviveTargetCoolingDown(_player)
            && TryMeasureNearestAiReviver(_player, honorFailedRevivers: true, out _))
        {
            return _player;
        }
        if (_aiReviveTarget is SquadMate currentTarget
            ...
            && !IsAiReviveTargetCoolingDown(currentTarget)
            && TryMeasureNearestAiReviver(currentTarget, honorFailedRevivers: true, out _))
        {
        ...
        SquadMate? nearestTarget = null;
        var nearestRescueDistanceSquared = float.PositiveInfinity;
        for (var targetIndex = 0; targetIndex < _squadMates.Count; targetIndex++)
        {
            var target = _squadMates[targetIndex];
            if (!IsInstanceValid(target)
                || target.IsHumanProxy
                || !target.CanBeRevived
                || IsAiReviveTargetCoolingDown(target))
            {
                continue;
            }
            // Candidate scoring remains geometric and bounded to the three squad
            // members. The committed route is still validated by the navigation
            // planner, but a low slot number can no longer pin a farther casualty.
            if (TryMeasureNearestAiReviver(
                    target,
                    honorFailedRevivers: true,
                    out var rescueDistanceSquared)
                && rescueDistanceSquared < nearestRescueDistanceSquared)
            {
                nearestRescueDistanceSquared = rescueDistanceSquared;
    ...
    private bool TryMeasureNearestAiReviver(
        ISquadCombatant target,
        bool honorFailedRevivers,
        out float distanceSquared)
    {
        distanceSquared = float.PositiveInfinity;
        if (!target.CanBeRevived || !IsInstanceValid(target.CombatNode))
        {
            return false;
        }
        ...
        foreach (var mate in _squadMates)
        {
            if (!CanAiRevive(mate, target)
                || honorFailedRevivers
                    && _failedAiReviversForTarget.Contains(mate.GetInstanceId()))
            {
                continue;
            }
            var candidateDistanceSquared = mate.GlobalPosition.DistanceSquaredTo(targetPosition);
            distanceSquared = Mathf.Min(distanceSquared, candidateDistanceSquared);
        }
        return !float.IsPositiveInfinity(distanceSquared);
    }

    private bool HasActionableAiReviveForSurvivorExtraction()
    {
        if (FindAiReviveTarget() is not null)
        {
            ClearAiRevivePreparation();
            return true;
        }

        // A critically injured survivor with medicine is still part of an
        // actionable rescue chain: pause evacuation, let them stabilize, then
        // retry the casualty instead of locking self-care behind the countdown.
        for (var targetIndex = 0; targetIndex < _squadMates.Count; targetIndex++)
        {
            var target = _squadMates[targetIndex];
            if (!IsInstanceValid(target)
            ...
            for (var mateIndex = 0; mateIndex < _squadMates.Count; mateIndex++)
            {
                var mate = _squadMates[mateIndex];
                if (!CanAiPrepareToRevive(mate, target))
                {
                    continue;
                }
                canPrepare = true;
                preparationInProgress |= mate.IsHealingForSquadRevivePreparation;
            }
            if (!canPrepare)
            {
                continue;
            ...
            if (_aiRevivePreparationTargetId != targetId)
            {
                _aiRevivePreparationTargetId = targetId;
                _aiRevivePreparationExpiry = _aiReviveRetryClock
                    + AiRevivePreparationTimeout;
                _aiRevivePreparationCompletionGraceUsed = false;
            }
            if (_aiReviveRetryClock < _aiRevivePreparationExpiry)
            {
                return true;
            }
            if (preparationInProgress && !_aiRevivePreparationCompletionGraceUsed)
            {
                // A treatment that began inside the safe window gets one bounded
                // chance to finish. Damage can still cancel it, and the grace is
                // never renewed, so evacuation cannot deadlock.
                _aiRevivePreparationCompletionGraceUsed = true;
                _aiRevivePreparationExpiry = _aiReviveRetryClock
                    + AiRevivePreparationCompletionGrace;
                return true;
            }

            // Self-care can be prevented indefinitely by nearby threats or
            // repeated damage. Cool down this casualty so one unsafe rescue does
            // not permanently trap all survivors.
            PutAiReviveTargetOnCooldown(target, AiReviveTargetRetryCooldown);
            ClearAiRevivePreparation();
        }
        ClearAiRevivePreparation();
    ...
    private void ClearAiRevivePreparation()
    {
        _aiRevivePreparationTargetId = 0;
        _aiRevivePreparationExpiry = 0.0f;
        _aiRevivePreparationCompletionGraceUsed = false;
    }

    private static bool CanAiPrepareToRevive(SquadMate mate, ISquadCombatant target)
        => IsInstanceValid(mate)
            ...
            && mate.CanSelfCareBeforeSquadRevive(AiReviveMinimumHealthRatio);

    private bool IsAiReviveTargetCoolingDown(ISquadCombatant target)
    {
        if (!IsInstanceValid(target.CombatNode))
        {
            return false;
        }
        var id = target.CombatNode.GetInstanceId();
        if (!_aiReviveTargetRetryCooldowns.TryGetValue(id, out var expiry))
        {
            return false;
        }
        if (expiry > _aiReviveRetryClock)
        {
            return true;
        }
        _aiReviveTargetRetryCooldowns.Remove(id);
        return false;
    }

    private void PutAiReviveTargetOnCooldown(ISquadCombatant target, float duration)
    {
        if (!IsInstanceValid(target.CombatNode))
        {
            return;
        ...
        var id = target.CombatNode.GetInstanceId();
        var expiry = _aiReviveRetryClock + Mathf.Max(0.0f, duration);
        var storedExpiry = Mathf.Max(
            expiry,
            _aiReviveTargetRetryCooldowns.GetValueOrDefault(id));
        _aiReviveTargetRetryCooldowns[id] = storedExpiry;
        _abandonedAiReviveTarget = target;
        _abandonedAiReviveTargetId = id;
        _abandonedAiReviveTargetExpiry = storedExpiry;
    }

    private void AdvanceAiReviveRetryClock(float delta)
    {
        _aiReviveRetryClock += Mathf.Max(0.0f, delta);
        if (_abandonedAiReviveTarget is null
            || _aiReviveRetryClock < _abandonedAiReviveTargetExpiry)
        {
            return;
        }
        _aiReviveTargetRetryCooldowns.Remove(_abandonedAiReviveTargetId);
        _abandonedAiReviveTarget = null;
        _abandonedAiReviveTargetId = 0;
        _abandonedAiReviveTargetExpiry = 0.0f;
    }
    ...
    private void ResetAiReviveAbandonment(ISquadCombatant? target = null)
    {
        if (target is not null && IsInstanceValid(target.CombatNode))
        {
            var targetId = target.CombatNode.GetInstanceId();
            _aiReviveTargetRetryCooldowns.Remove(targetId);
            if (_abandonedAiReviveTargetId == targetId)
            {
                _abandonedAiReviveTarget = null;
                _abandonedAiReviveTargetId = 0;
                _abandonedAiReviveTargetExpiry = 0.0f;
        ...
        _abandonedAiReviveTargetId = 0;
        _abandonedAiReviveTargetExpiry = 0.0f;
        _aiReviveRetryClock = 0.0f;
        _aiReviveTargetRetryCooldowns.Clear();
        ClearAiRevivePreparation();
        _reviverNoProgressTime = 0.0f;
    }
    ...
    private void TryIssuePlayerSquadOrder(int value)
    {
        if (value < (int)SquadOrder.Follow || value > (int)SquadOrder.Move)
        {
            return;
        }
        TryIssuePlayerSquadOrder((SquadOrder)value);
    }

    private bool TryIssuePlayerSquadOrder(SquadOrder order)
    {
        if (!CanLocalPlayerIssueAiSquadCommands)
        {
            return false;
    ...
    private void TryTogglePlayerSquadFireStance()
        => TryTogglePlayerSquadFireStanceForDiagnostics();

    private bool TryTogglePlayerSquadFireStanceForDiagnostics()
    {
        if (!CanLocalPlayerIssueAiSquadCommands)
        {
            return false;
    ...
    private void TryMarkPlayerSquadFocusTarget()
        => TryMarkPlayerSquadFocusTargetForDiagnostics();

    private bool TryMarkPlayerSquadFocusTargetForDiagnostics()
    {
        if (!CanLocalPlayerIssueAiSquadCommands)
        {
            return false;
        ...
        return _squadFocusTarget is not null
            && IsInstanceValid(_squadFocusTarget)
            && !_squadFocusTarget.IsDead;
    }
    ...
    private void IssueSquadOrder(SquadOrder order)
    {
        ...
        {
            _squadMovePoint = ResolveSquadCommandAimPoint(65.0f);
            ShowSquadMoveMarker(_squadMovePoint);
        }
        else if (order == SquadOrder.Hold)
        {
            _squadMovePoint = CanEliminatedPlayerCommandLocalAi
                ? ResolveSquadFollowAnchor().GlobalPosition
                : _player.GlobalPosition;
        }
        foreach (var mate in _squadMates)
        {
    ...
    private Vector3 ResolveSquadCommandAimPoint(float maximumDistance)
    {
        if (!TryGetEliminatedSquadCommandView(out var origin, out var direction))
        {
            return _player.GetAimPoint(maximumDistance);
        }
        var target = origin + direction * maximumDistance;
        var exclusions = new Godot.Collections.Array<Rid> { _player.GetRid() };
        using var exclusionsBacking = exclusions.AsDisposable();
        if (_spectatedMate is not null && IsInstanceValid(_spectatedMate))
        {
            exclusions.Add(_spectatedMate.GetRid());
        }
    ...
    private bool TryGetEliminatedSquadCommandView(
        out Vector3 origin,
        out Vector3 direction)
    {
        if (CanEliminatedPlayerCommandLocalAi
            ...
            && _squadSpectatorCamera.Current)
        {
            origin = _squadSpectatorCamera.GlobalPosition;
            direction = -_squadSpectatorCamera.GlobalBasis.Z.Normalized();
            return true;
        }
        origin = default;
        direction = default;
    ...
    {
        var spectatorView = TryGetEliminatedSquadCommandView(out var origin, out var direction);
        if (!spectatorView)
        {
            origin = _player.GlobalPosition + Vector3.Up * 1.55f;
            direction = -_player.GlobalBasis.Z;
        }
    ...
    private void RefreshSquadHud()
    {
        ...
        var survivorCommandsAvailable = CanEliminatedPlayerCommandLocalAi;
        _hud.SetSquadCommandPresentation(
            survivorCommandsAvailable,
            survivorCommandsAvailable,
            _localPlayerEliminated && !survivorCommandsAvailable);
    }
    ...
    public ISquadCombatant? FindLowestFriendly(float healthRatio, bool includeDowned)
    {
        ...
        foreach (var friendly in FriendlyCombatants())
        {
            if (friendly.CombatDead && (!includeDowned || !friendly.CanBeRevived))
            {
                continue;
            }
    ...
    public void ApplyMedicSpray(ISquadCombatant source, Vector3 origin, Vector3 forward)
    {
        ...
        foreach (var friendly in FriendlyCombatants())
        {
            if (friendly.CombatDead && !friendly.CanBeRevived)
            {
                continue;
            }
    ...
    public void SpawnSquadBodyBag(SquadMate mate)
    {
        ...
        // Preserve the operator's actual upgraded kit and unused sustainment stock;
        // the small ammunition stack remains the baseline recoverable field supply.
        bag.Loot.Add(new LootItem { Kind = LootItemKind.Ammunition, Quantity = 30 });
        if (!mate.IsHumanProxy && !mate.IsNetworkProxy)
        {
            // Only field-recovered AI items are appended. Free baseline kit is not
            // sellable, and network proxies do not invent a local AI inventory.
            mate.AppendSustainmentLoot(bag.Loot);
        }
        AddChild(bag);
    ...
    /// <summary>
    /// A downed leader no longer owns the squad order state. Clear every stale
    /// player-issued directive and let the normal AI rescue state machine choose
    /// the nearest capable rescuer.
    /// </summary>
    private void ApplyAutonomousRescueSquadState()
    {
        _squadOrder = SquadOrder.Follow;
        ...
        _squadMoveMarker = null;
        ClearLeaderReviveAi();
        foreach (var mate in _squadMates)
        {
            if (!IsLocalAiMate(mate))
            {
                continue;
            }
            mate.SetHoldFire(false);
            mate.SetOrder(SquadOrder.Follow, mate.GlobalPosition);
            ClearSquadNavigation(mate);
        }
        _hud.SetSquadOrder(SquadOrder.Follow);
        _hud.SetSquadFireStance(false);
        _hud.SetSquadCommandPresentation(false, false);
    }
    ...
    private bool HandleLocalPlayerDowned()
    {
        ...
        ResetAiReviveAbandonment(_player);
        _localPlayerDowned = true;
        _localPlayerDownedTimer = 0.0f;
        if (_extractionCountdownActive)
        {
            CancelExtractionCountdown(announce: false);
        }
        ApplyAutonomousRescueSquadState();
        _player.UiLocked = true;
        _player.DisarmFireInput();
        _player.DisarmMovementInput();
    ...
    private bool TryBeginLocalPlayerElimination()
    {
        ...
        var aiCommandAvailable = CanEliminatedPlayerCommandLocalAi;
        _hud.SetSquadCommandPresentation(
            aiCommandAvailable,
            aiCommandAvailable);
        if (aiCommandAvailable)
        {
            ResetSquadLeaderTrail(ResolveSquadFollowAnchor().GlobalPosition);
            _hud.ShowLocalizedMessage(
                "spectator_ai_command_toast",
                "AI COMMAND ACTIVE  //  F1-F5 REMAIN AVAILABLE",
                new Color(0.38f, 0.9f, 0.82f));
        }
        return true;
    }
    ...
    private async void ValidateSquadFlow()
    {
        ...
        var spectatorCycle = ValidateSquadSpectatorCycleForDiagnostics(finishTarget);
        ...
        mateReviver.Velocity = Vector3.Zero;
        mateReviver.SetSustainmentStateForDiagnostics(
            mateReviver.MaxHealth * 0.3f,
            armorRatio: 0.0f,
            bandages: 0,
            plates: 0);
        ...
        var reverseTrailInRange = float.IsFinite(reverseTrailCost);
        _localPlayerEliminated = true;
        _player.UiLocked = true;
        ...
        PutAiReviveTargetOnCooldown(_player, AiReviveTargetRetryCooldown);
        _player.SetHealthForDiagnostics(10.0f);
        _player.SetReviveUsedForDiagnostics(false);
        _player.TakeDamage(999.0f, _player.HitPoint(HitRegion.Torso), this);
        if (!_player.IsDead)
        ...
        var aliveAiCommandHintHidden = !_hud.IsAiSquadCommandPresentationVisibleForDiagnostics
            && !_hud.AreSquadCommandControlsVisibleForDiagnostics;
        var aiCommandLocalOnly = SquadCommandRules.CanExposeAiCommands(false, false, true, 2)
            && !SquadCommandRules.CanExposeAiCommands(false, true, true, 2)
            && !SquadCommandRules.CanExposeAiCommands(true, false, true, 4)
            && !SquadCommandRules.CanExposeAiCommands(false, false, false, 2)
            && !SquadCommandRules.CanExposeAiCommands(false, false, true, 0);
        var firstDownAiCommandHintHidden = false;
        var secondDeathAiCommandHintVisible = false;
        var eliminatedOrdersAccepted = false;
        var eliminatedMoveUsesSpectatorView = false;
        var eliminatedFollowUsesAiLeader = false;
        var eliminatedFireToggleAccepted = false;
        var eliminatedFocusAccepted = false;
        var eliminatedHintLocalized = false;
        var firstDownCommandsCancelled = false;
        var allAiSecondDownFailed = false;
        var secondDownRuleAllAi = SquadReviveRules.ShouldFailExtractionOnSecondDown(
        ...
        if (reviverMate is not null)
        {
            ...
            firstDownAiCommandHintHidden = !_hud.IsAiSquadCommandPresentationVisibleForDiagnostics
                && !_hud.AreSquadCommandControlsVisibleForDiagnostics
        ...
        var gridPathClearedByAutonomousRescue = false;
        if (gridMate is not null)
        {
            ...
            gridPathLifecycleOk = gridPathCreated
                ...
                && gridFollowPath.Cursor >= 0
                && gridFollowPath.Cursor < gridFollowPath.Directives.Length;
            ...
            gridPathClearedByAutonomousRescue = !_squadGridPaths.ContainsKey(
            ...
            gridRescueCompleted = gridRescueDowned
                ...
                && _player.ReviveUsed
                && !_localPlayerDowned;
            gridEmergencyPathReady = gridReviverAssigned
                && gridPathClearedByAutonomousRescue
                && gridRescueUsedGrid;
        ...
        // A downed leader relinquishes stale player orders immediately. Seed a
        // visible Hold + hold-fire state so the validator proves the reset path.
        if (_squadHoldFire)
        {
            ToggleSquadFireStance();
        }
        ...
        var localAiAfterDown = _squadMates.Where(IsLocalAiMate).ToArray();
        firstDownCommandsCancelled = playerDowned
            && _squadOrder == SquadOrder.Follow
            && !_squadHoldFire
            && localAiAfterDown.All(mate => mate.Order == SquadOrder.Follow && !mate.HoldFireActive)
            && !CanLocalPlayerIssueAiSquadCommands
            && !_hud.IsAiSquadCommandPresentationVisibleForDiagnostics
            && !_hud.AreSquadCommandControlsVisibleForDiagnostics
            && _hud.DownedFooterSuppressedForDiagnostics;
        var playerFirstRevive = _player.TryReceiveRevive(50.0f);
        var playerUp = !_player.IsDead && playerFirstRevive && _player.ReviveUsed;
        // Keep a mixed-human roster for the legacy spectator command path. The
        // production all-AI second-down path is verified after this block.
        SquadMate? diagnosticHumanProxy = null;
        if (playerUp)
        {
            diagnosticHumanProxy = SpawnSquadMate(
                99,
        ...
        var secondDeathSpectate = TryBeginLocalPlayerElimination()
            && _localPlayerEliminated
            && IsSquadMateViewCurrent
            && !_missionEnded;
        secondDeathAiCommandHintVisible = CanEliminatedPlayerCommandLocalAi
            && _hud.IsAiSquadCommandPresentationVisibleForDiagnostics
            && _hud.AreSquadCommandControlsVisibleForDiagnostics;
        var localCommandMates = _squadMates.Where(IsLivingLocalAiMate).ToArray();
        var holdAcceptedAfterElimination = TryIssuePlayerSquadOrder(SquadOrder.Hold);
        var allHeldAfterElimination = localCommandMates.Length > 0
            && localCommandMates.All(mate => mate.Order == SquadOrder.Hold);
        var expectedSpectatorMovePoint = ResolveSquadCommandAimPoint(65.0f);
        var moveAcceptedAfterElimination = TryIssuePlayerSquadOrder(SquadOrder.Move);
        eliminatedMoveUsesSpectatorView = moveAcceptedAfterElimination
            && _squadMovePoint.DistanceTo(expectedSpectatorMovePoint) < 0.08f
            && _squadSpectatorCamera is not null
            && IsInstanceValid(_squadSpectatorCamera)
            && (_squadMovePoint - _squadSpectatorCamera.GlobalPosition).Normalized()
                .Dot(-_squadSpectatorCamera.GlobalBasis.Z) > 0.72f;
        var followAcceptedAfterElimination = TryIssuePlayerSquadOrder(SquadOrder.Follow);
        var followAnchor = ResolveSquadFollowAnchor();
        eliminatedFollowUsesAiLeader = followAcceptedAfterElimination
            && followAnchor is SquadMate anchorMate
            && IsLivingLocalAiMate(anchorMate)
            && localCommandMates.All(mate => mate.Order == SquadOrder.Follow);
        eliminatedOrdersAccepted = holdAcceptedAfterElimination
            && allHeldAfterElimination
            && moveAcceptedAfterElimination
            && followAcceptedAfterElimination;
        var holdFireBeforeEliminatedCommand = _squadHoldFire;
        eliminatedFireToggleAccepted = TryTogglePlayerSquadFireStanceForDiagnostics()
            && _squadHoldFire != holdFireBeforeEliminatedCommand
            && localCommandMates.All(mate => mate.HoldFireActive == _squadHoldFire);
        if (_squadHoldFire != holdFireBeforeEliminatedCommand)
        {
            ToggleSquadFireStance();
        }
        var focusProbe = _enemies.FirstOrDefault(enemy =>
            IsInstanceValid(enemy) && !enemy.IsDead);
        if (focusProbe is not null
            && TryGetEliminatedSquadCommandView(out var focusOrigin, out var focusDirection))
        {
            focusProbe.ResetTacticalStateForDiagnostics();
            focusProbe.SetAlerted(focusOrigin);
            focusProbe.ProcessMode = ProcessModeEnum.Disabled;
            focusProbe.GlobalPosition = focusOrigin + focusDirection * 18.0f - Vector3.Up;
            ClearSquadFocusTarget();
            eliminatedFocusAccepted = TryMarkPlayerSquadFocusTargetForDiagnostics()
                && ReferenceEquals(_squadFocusTarget, focusProbe);
            ClearSquadFocusTarget();
        }
        _hud.SetLanguage("en");
        var englishAiCommandHint = _hud.SquadCommandTitleForDiagnostics.Contains(
                "AI COMMAND",
                StringComparison.Ordinal)
            && _hud.SquadCommandHintForDiagnostics.Contains("F1", StringComparison.Ordinal)
            && _hud.SquadCommandHintForDiagnostics.Contains("F5", StringComparison.Ordinal);
        _hud.SetLanguage("zh");
        var chineseAiCommandHint = _hud.SquadCommandTitleForDiagnostics.Contains(
                "\u6307\u6325",
                StringComparison.Ordinal)
            && _hud.SquadCommandHintForDiagnostics.Contains("F1", StringComparison.Ordinal)
            && _hud.SquadCommandHintForDiagnostics.Contains("F5", StringComparison.Ordinal);
        eliminatedHintLocalized = englishAiCommandHint && chineseAiCommandHint;
        ...
        {
            ClearDemolitionSquadMateState(diagnosticHumanProxy);
            _squadMates.Remove(diagnosticHumanProxy);
            diagnosticHumanProxy.QueueFree();
        }
        // With the synthetic human proxy removed, the same second down must
        // now end an all-AI extraction instead of opening the spectator UI.
        _player.SetHealthForDiagnostics(10.0f);
        _player.SetReviveUsedForDiagnostics(true);
        OnLocalPlayerRevived();
        var allAiSecondDownDamage = _player.TakeCombatDamage(
            999.0f,
            _player.HitPoint(HitRegion.Torso),
            this);
        allAiSecondDownFailed = allAiSecondDownDamage
            && _missionEnded
            && !_localPlayerDowned
            && !_localPlayerEliminated
        ...
        var valid = ActiveSquadCount >= 2 && roleFillOk && combatAiOk
            ...
            && aiCommandLocalOnly && aliveAiCommandHintHidden && firstDownAiCommandHintHidden
            && secondDeathAiCommandHintVisible && eliminatedOrdersAccepted
            && eliminatedMoveUsesSpectatorView && eliminatedFollowUsesAiLeader
            && eliminatedFireToggleAccepted && eliminatedFocusAccepted
            && eliminatedHintLocalized && finishedSpectateOk
            && secondDownRuleAllAi && allAiSecondDownFailed
