...
每次部署都由三名干员组成。玩家在部署界面选择突击兵、医疗兵或侦察兵，另外两个位置由 AI 自动补齐。可以选择 `LOCAL + AI`、`HOST GAME`，或在 `JOIN GAME` 中输入主机地址。真人玩家连接后会替换一个 AI 槽位；断线时该槽位交还 AI，不会重启任务。联机使用 UDP 端口 `28960` 上的 ENet，因此主机可能需要在 Windows 防火墙中允许本游戏。加入地址可以是 `host`（默认端口 `28960`）或 `host:port`，也支持 UDP 隧道分配的公网端点。免费 playit.gg 配置和替代方案请参阅 [ONLINE_PLAY.md](ONLINE_PLAY.md)。
