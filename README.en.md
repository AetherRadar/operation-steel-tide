...
- **A second competitive ruleset:** browse a twelve-map pool with Bazaar Crossing, Tideglass Reactor, Tideforge Arena, and Harbor Locks currently playable in that order, then enter MR12 5v5 rounds with manual purchases, side swaps, overtime, planting, defusing, and role-aware tactical AI.
...
### Current Chinese Gameplay Captures

The following images are generated directly by Godot from the current Jianghai Old City build. They preserve the live Chinese HUD, first-person weapons, and actual gameplay interfaces.

![First-person firefight on a Jianghai Old City avenue](docs/media/gameplay-combat-zh.webp)

**Close firefight.** The player engages two operators with the M4A1 while the live weapon pose, muzzle flash, minimap, and combat state remain visible.

![Player advancing with two AI squadmates near the temple](docs/media/gameplay-squad-zh.webp)

**Squad coordination.** Two AI teammates move through the temple district while the Chinese HUD reports roles, health, and squad state.

![Chinese tactical HUD overlooking the elevated market](docs/media/gameplay-tactical-zh.webp)

**Tactical picture.** The elevated market, objective, compass, minimap, ammunition, and quick slots share one live frame.

![Chinese personal backpack and equipped-loadout interface](docs/media/gameplay-loot-zh.webp)
...
- `1` draws the primary weapon, `2` the secondary weapon, `3` the sidearm, `4` the equipped melee weapon, `5` a frag grenade, and `6` the current utility item (a smoke grenade); grenade and utility slots stay hidden at zero inventory, and the lower-right slots can also be clicked while the pointer is available
- Left mouse fires or chains up to three melee cuts; while alive, right mouse aims through the installed optic, and while downed or eliminated it cycles through living squadmate views; `R` performs a full magazine reload
...
The Operations Office also launches the separate demolition match from a twelve-map pool. Its briefing shows one map at a time with previous/next controls: `BAZAAR CROSSING`, `TIDEGLASS REACTOR`, `TIDEFORGE ARENA`, and `HARBOR LOCKS` occupy the first four playable slots, while the other eight remain visibly locked until their geometry ships. Tideglass Reactor is now `MAP 02`: its 136 m x 112 m combat space uses a compact middle and two longer wings, with Construction Court A and Old Reactor Loading Yard B as the only two device sites, separated by 95.52 m. Forty-one major buildings cover all five districts, including 30 expansion buildings. Its construction tower and crane, complete brick reactor hall, civic crossroad, distinct perimeter gates, residential offices, orange-gray modular halls, and street furniture form continuous streets instead of detached partition clutter. Twenty-six dressing-model references plus 52 collision-backed authored-prop references make 78 total references and 70 distinct authored scene paths from eight CC0 source collections; repeated references only reuse complete building models. Three attack lanes and all six ground routes pass capsule-clear and runtime-physics checks, with minimum lane separations of 35.22 / 9.90 / 30.77 m and zero overlap. Strict density sampling caps the longest horizontal sightline at 58.00 m and measures the largest open-space diameter at 21.78 m. The map builds in under 6000 ms, while every cover point, strategy target, site-cover check, and physical spawn/site occlusion check passes. Tideforge splits its sites across an open foundry and enclosed assembly hall. Harbor Locks recomposes Kenney's CC0 City Kit (Industrial) models into a lock-gate district with pump stations, control buildings, two long quayside channels, three attack routes, and hard-cover rotations. Bazaar Crossing compresses a three-lane old-city market, a committed kinked long route, and a bent east approach into a 136 m x 112 m combat space. Three purpose-built upper positions join the three-dimensional AI graph through six bidirectional stairs while both sites remain at street level for unambiguous planting and defusing.

Each match is MR12 5v5: the player plus four AI teammates against five opponents, first to 13 rounds, halftime side swap with wallets reset to $800, and a win-by-two overtime that swaps sides every four rounds. A 15-second buy phase freezes combat before every round and presents sidearms, primary weapons, armor, and grenades with exact prices. The opening $800 can buy a P226 or M1911 but cannot buy a primary; confirming deducts the validated total once, while timeout accepts the current affordable selection or starts knife-only. Round wins pay $3,000, losses start at $1,900 plus a $500 loss-streak escalation, planting or defusing pays $300, and wallets cap at $9,000. Opponents purchase from the same price ladder, while extraction progression and its wallet remain isolated. When the player squad defends, enemy AI picks a carrier who walks a route, plants, and the remaining attackers hold angles; attackers defuse through cover while defenders rotate.
...
- `csharp/DemolitionArenaLayout*.cs`, `DemolitionArenaBuilder*.cs`, and `DemolitionArenaRuntime.cs`: Tideforge, Harbor Locks, Tideglass Reactor, and Bazaar Crossing geometry data, authored multi-pack scene composition, invisible gameplay collision, activation isolation, three-dimensional routes, elevated rotations, and minimap markers.
...
Godot_console.exe --path . -- --validate-tideglass-density
...
`--validate-demolition` verifies the Operations Office entry, role and map selection, the frozen buy phase before live combat, exact opening-pistol spending, sidearm-only firing, empty protection/utility state, isolated economy and extraction systems, the 5v5 squad fill, opponent purchases, opening and post-plant AI duties, planting, physical AI defusing, the tactical AI layer (combat-first arbitration with hysteresis, smoke-aware target loss, safe-frontier retries, carrier/defuser route recovery, clock-pressure site switching, and squad post anchoring), round rewards, round scoring/reset, the round-13 halftime side swap with enemy-carrier planting and player defusing, MR12/13-win/overtime win-by-two rules, and the economy reward table. `--validate-demolition-rules` verifies the demolition-only HUD, recon boundary, utility binding, elimination collision state, and localized spectator flow. `--validate-demolition-arena` checks Tideforge activation, 108 m spawn separation, roughly 80-90 m A/B approaches, the roughly 113 m rotation, balanced travel, capsule-clear routes, blocked spawn sightlines, 77 m site separation, site placement, localized minimap markers, and spatial isolation. `--validate-harbor-locks` checks the fourth selectable map, all imported CC0 industrial-model instances, localization, collision lifecycle, and deterministic routes to both sites; `--capture-harbor-locks` renders its tactical overview. `--validate-tideglass-density` independently enforces the second map's 136 m x 112 m footprint, exactly two device sites separated by at least 80 m, 41 major buildings across five regions (including 30 expansion buildings), 52 collision-backed authored props, 78 authored references with 70 distinct scene paths, eight CC0 source packs, a 58.00 m longest horizontal sightline, a 21.78 m largest open-space diameter, zero lane overlap, six physical routes, no detached visible partitions, reachable cover/strategy targets, and physical occlusion from both teams' spawns to A/B, between spawns, and between the two sites. `--validate-tideglass-reactor` further checks 26 dressing-model references plus 52 collision-backed authored-prop references, controlled reuse of complete building models, opaque solid materials with explicit glass/window exceptions, tightly model-aligned collision, scale-baked authored ground and landmark trimeshes, closed modular-building structure, visible perimeter-gate alignment, exact authored collision and player traversal for both elevated-walkway stairs, physical spawn sightline blocking, clear cover and strategy targets, and all six routes against both layout and runtime physics; `--capture-tideglass-reactor` renders overview and player-height views covering both sites, three lanes, both spawns, major buildings, the elevated walkway, arch gateway, and both perimeter gates. `--validate-bazaar-crossing` deterministically checks the old-city three-lane structure, six bidirectional stairs, three-dimensional player/squad/opponent routes, spawn/site sightline blocking, authored GLB materials and UVs, and runtime lifecycle; `--capture-bazaar-crossing` renders overhead, oblique, site, lane, upper-deck, stair, and spawn views. `--validate-demolition-briefing` verifies scene loading, required bindings, English and Chinese synchronization, four playable maps in the twelve-entry carousel, locked-map rejection, role/map selection without deployment, and back/deploy intent signals. `--validate-demolition-buy` verifies the pure price rules, opening-primary lock, P226 and smoke-grenade costs, unaffordable-selection blocking, authored scene bindings, English and Chinese synchronization, live HUD state, and purchase intent payload.
