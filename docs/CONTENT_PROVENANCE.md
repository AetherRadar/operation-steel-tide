...
| Authored weapon platform `.blend` and `.glb` models (AK-74N, SCAR-L, M24, MP5A5, M3A1, AXMC, AWM, VSS, P226, M1911) | Project-authored in Blender from the checked-in modeling script, with disclosed AI assistance; no third-party geometry or textures copied | Included under the root MIT license, subject to the limitation above | `scripts/blender/generate_weapon_platform_models.py`, `source_art/weapon_platforms/`, and `assets/models/weapon_platforms/` |
