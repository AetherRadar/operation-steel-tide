...
| Female field operator and its retargeted action set | Quaternius Ultimate Modular Women `Soldier` model plus Quaternius Universal Animation Library actions | CC0 1.0 Universal; no attribution required, creator credit retained as courtesy | Model acquired 2026-08-27 from `https://quaternius.com/packs/ultimatemodularwomen.html`; exact source mapping, supplied license evidence, editable Blender source, and runtime GLB are recorded in `assets/models/quaternius_female_operator/LICENSE.md` and `source_art/third_party/quaternius_modular_women/` |
